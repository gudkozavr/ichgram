...
interface MediaSliderProps {
  ...
  likes: string[];
  onClick: () => void;
  compactMode?: boolean; // Добавляем проп для условного рендеринга
}
...
const MediaSlider: React.FC<MediaSliderProps> = ({
  ...
  likes,
  onClick,
  compactMode = false, // По умолчанию компонент будет отрисовываться полностью
}) => {
  ...
  if (compactMode) {
    return (
      <div
        className="relative flex items-center justify-center w-full bg-black h-80"
        onClick={onClick}
      >
        {media[currentIndex] &&
